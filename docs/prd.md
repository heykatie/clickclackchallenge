...
This file owns product behavior: scoring, the accuracy gate, ranking, nickname rules, continue-event duration, reset timing, what must persist, and the requirement that the booth works offline.
...
Booth acceptance tests, the automated test map, and the giant-keyboard and airplane-mode checks live in `docs/testing.md`.

V1 is not event-ready until those checks pass, including the airplane-mode test on the target iPad and giant keyboard.
